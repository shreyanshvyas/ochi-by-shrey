...
const Navbar = () => {
  ...
  return (
    ...
    <div  className="fixed z-[5] w-full px-20 py-5 flex justify-between items-center backdrop-blur-sm">
      <div className="logo">
        <svg
          width="72"
          height="30"
      ...
      <div className="links flex gap-10">
        {["Services", "Our Works", "About Us", "Insights", "Contact Us"].map((item,index) => (
          <a key={index} className={`text-lg font-extralight font-['Neue_Montreal'] ${index === 4 &&"ml-32"}`}>{item}</a>
        ))}
      </div>
    </div>
    </nav>
